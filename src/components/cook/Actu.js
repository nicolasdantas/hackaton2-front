...
export default function Actu({ showRecipe, setShowRecipe }) {
  ...
  useEffect(() => {
    axios.get('http://526037743aa4.ngrok.io/api/users/').then((res) => {
  ...
  const handleSubmit = (e, data) => {
    axios
      .post('http://d5b70dbe70d0.ngrok.io/api/lunches/new', data)
  ...
  return (
    <div className='container-page-actu'>
      ...
      <form className='container-form-actu' onSubmit={handleSubmit}>
        <h2 style={{ color: 'white' }} className='title-recette'>
          Publie ta recette !
        </h2>
        ...
        <div className='form-post-actu'>
          ...
          <div className='div-button'>
            <button className='button-actu' type='submit'>
              Publier
            </button>
          </div>
        </div>
                ...
                {post.lunches.length > 0 && (
                  <div className='cards-container' key={post.id}>
                    <div className='infos-recipes'>
                      ...
                      <p style={{ color: 'white' }}>
                        {dayjs(post.lunches[0].created_at).format('DD/MM/YY')}
                      </p>
                      <img
                        className='image-recipe'
                        src={post.lunches[0].image}
                        alt={post.lunches[0].name}
                      />
                      <p style={{ color: 'white' }}>
                        {post.lunches[0].content}
                      </p>
                      <Box
                        component='fieldset'
                        mb={3}
